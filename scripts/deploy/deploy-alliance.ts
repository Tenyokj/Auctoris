...
async function main() {
  ...
  console.log("FATK deployed:", tokenAddress);
  ...
  if (process.env.CREATE_SAMPLE_ALLIANCE === "true") {
    ...
    const quorumPercent = BigInt(process.env.SAMPLE_QUORUM_PERCENT ?? "60");
    const lossSaleQuorumPercent = BigInt(process.env.SAMPLE_LOSS_SALE_QUORUM_PERCENT ?? "80");
    const minSalePrice = BigInt(process.env.SAMPLE_MIN_SALE_PRICE_WEI ?? targetPrice);
