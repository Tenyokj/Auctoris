...
SAMPLE_SHARES=60,40 \
SAMPLE_QUORUM_PERCENT=60 \
SAMPLE_LOSS_SALE_QUORUM_PERCENT=80 \
SAMPLE_MIN_SALE_PRICE_WEI=1000000000000000000000 \
npx hardhat run scripts/deploy/deploy-alliance.ts --network localhost

# 4) (Optional) Deploy mock NFT
npx hardhat run scripts/deploy/deploy-mock.ts --network localhost

# 5) (Optional) Deploy FATKFaucet
TOKEN_ADDRESS=0xYourFATK \
FAUCET_OWNER=0xYourOwner \
MINT_FAUCET_LIQUIDITY=true \
FAUCET_LIQUIDITY_AMOUNT=1000000 \
npx hardhat run scripts/deploy/deploy-fatkfaucet.ts --network localhost
```
...
7. `SAMPLE_QUORUM_PERCENT` (default `60`)
8. `SAMPLE_LOSS_SALE_QUORUM_PERCENT` (default `80`)
9. `SAMPLE_MIN_SALE_PRICE_WEI` (defaults to `SAMPLE_TARGET_PRICE_WEI`)
...
Use only these scripts for FlashAlliance:
1. `scripts/deploy/deploy-alliance.ts`
2. `scripts/deploy/deploy-mock.ts`
3. `scripts/deploy/deploy-fatkfaucet.ts`

Legacy duplicates under `scripts/deploy/*` are marked as deprecated and should be avoided for FlashAlliance.
...
**Deploy FATKFaucet**
Required env vars:
1. `TOKEN_ADDRESS` (`FATK` token address)

Optional env vars:
1. `FAUCET_OWNER` (default deployer)
2. `FAUCET_CLAIM_AMOUNT` (default `1000`, token units with 18 decimals)
3. `FAUCET_COOLDOWN_SEC` (default `86400`)
4. `MINT_FAUCET_LIQUIDITY` (`true`/`false`, default `false`)
5. `FAUCET_LIQUIDITY_AMOUNT` (default `1000000`, used only when mint flag is enabled)
...
**Deploy ERC721Mock**
Optional env vars:
1. `MOCK_NFT_NAME` (default `FlashAlliance Mock NFT`)
2. `MOCK_NFT_SYMBOL` (default `FAMOCK`)
3. `MOCK_NFT_TOKEN_ID` (default `1`)
4. `MOCK_NFT_SELLER` (default second signer)
5. `APPROVE_ALLIANCE` (`true`/`false`, default `false`)
6. `ALLIANCE_ADDRESS` (required only if `APPROVE_ALLIANCE=true`)
